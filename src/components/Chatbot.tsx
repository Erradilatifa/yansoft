      ...
      >
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col items-center text-center max-w-3xl mx-auto animate-fade-up">
            ...
            <div className="flex flex-wrap justify-center gap-8 mt-4 mb-12 animate-fade-in" style={{ animationDelay: '300ms' }}>
              {[
                { icon: <FileText className="h-5 w-5" />, text: "Devis détaillé" },
                { icon: <Clock className="h-5 w-5" />, text: "Réponse rapide" },
                { icon: <Users className="h-5 w-5" />, text: "Expertise technique" },
                { icon: <MessageSquare className="h-5 w-5" />, text: "Conseil personnalisé" },
              ].map((item, index) => (
                <div 
                  key={index} 
                  className="flex items-center gap-2 text-sm text-muted-foreground bg-white/80 backdrop-blur-sm rounded-full px-4 py-2 shadow-sm"
                >
                  {item.icon}
                  <span>{item.text}</span>
                </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-1/2">
              <div 
                id="processus" 
                ref={processRef}
                className="py-8 px-4 md:px-8 relative bg-image-overlay overflow-hidden rounded-2xl h-full"
                style={{
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="text-center mb-8">
                    <span className="inline-block text-xs font-medium bg-blue-500/10 text-blue-500 px-3 py-1 rounded-full mb-3">
                      Notre processus
                    </span>
                  ...
                  <div className="py-4 relative">
                    {/* Ligne verticale avec animation GSAP */}
                    ...
                    <div className="space-y-8 relative">
                      {steps.map((step, index) => (
                        ...
                        >
                          <div className={`w-full md:w-2/5 ${index % 2 === 0 ? 'md:text-right' : 'md:text-left'} bg-white bg-opacity-80 rounded-xl p-3 shadow-md`}>
                          ...
                          <div className={`w-full md:w-2/5 ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'} hidden md:block`}>
            ...
            {/* Formulaire */}
            <div className="lg:w-1/2">
              <div className="glass-card rounded-2xl p-8 shadow-lg h-full">
                <div className="text-center mb-10 animate-fade-up">
                  <h2 className="text-3xl font-bold mb-4">Remplissez le <span className="text-blue-500">formulaire</span></h2>
